...
from Post.models import Item, Offer
from django.template import loader
from django.urls import reverse

def viewProfile(request, userID):
	user = get_object_or_404(User, pk = userID)
	item_list = Item.objects.filter(item_userfk = userID).order_by('-id')
	offer_list = Offer.objects.filter(offer_itemfk__item_userfk__id = userID)
	if user.id == request.session["user"]:
		return render(request, 'Profile/myProfTemp.html', {'user': user, 'item_list':item_list, 'offer_list':offer_list,})
	return render(request, 'Profile/profTemp.html', {'user': user,'item_list':item_list,})
